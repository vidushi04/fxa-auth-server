  ...
  const routes = [
    ...
    },
    {
      ...
      options: {
        ...
        response: {
          schema: {
            ...
            sessionToken: isA.string().regex(HEX_STRING).required(),
            keyFetchToken: isA.string().regex(HEX_STRING).optional(),
            verificationMethodOptional: isA.boolean().optional(),
      ...
      handler: async function (request) {
        ...
        const verificationMethodOptional = request.payload.verificationMethodOptional || false
        ...
        function createSessionToken () {
          ...
          // used for sync.
          const mustVerifySession = needsVerificationId && (requestHelper.wantsKeys(request) || (verificationMethod && ! verificationMethodOptional))
        ...
        function forceTokenVerification (request, account) {
          ...
          // If relier has specified a verification method and it is specified as forced, we should use it
          if (verificationMethod && ! verificationMethodOptional) {
            return true
          }
